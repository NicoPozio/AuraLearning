/**
 * CalibrationUI — full-screen overlay for the 9-point gaze calibration.
 *
 * Responsibility split with main.js:
 *   - This class handles presentation only: showing the dots, drawing
 *     the progress ring, transitioning between the intro screen and
 *     the dot sequence.
 *   - main.js handles the actual sample collection (frame-by-frame
 *     iris readings while SPACE is held) and calls back into
 *     updateProgress() and advance() at the right moments.
 *
 * Lifecycle:
 *   start()           — overlay visible, intro screen
 *   onSpaceDown()     — intro → dots transition
 *   updateProgress(p) — animate the ring fill while SPACE is held
 *   advance()         — move to the next dot (called by main.js when
 *                       enough samples have been collected)
 *   onComplete()      — fired after the 9th dot is recorded
 */
export class CalibrationUI {

    /**
     * @param {() => void} onComplete - Invoked once the user has
     *        confirmed all 9 anchors. Typical implementation: trigger
     *        the TPS fit and reveal the gaze dot.
     */
    constructor(onComplete) {
        this.onComplete = onComplete;
        // Anchor layout: 4 corners, centre, 2 edge midpoints, and 2
        // lower-band off-axis anchors. The lower-band anchors (y=0.65)
        // are intentionally off-axis: they help the TPS interpolate
        // the reading area of a PDF, which typically sits in the lower
        // half of the screen.
        this.points = [
        ...
        this._ringEl = null;     // SVG <circle> currently being animated
        this._ringPerim = 0;     // cached perimeter for stroke-dashoffset
        this._labelEl = null;    // bottom instruction label
    }
    ...
    _createOverlay() {
        ...
        // cursor:none hides the OS pointer during calibration so it
        // doesn't distract from the actual dot.
    ...
    /**
     * Handle a SPACE keydown from main.js. The first press dismisses
     * the intro and shows the first dot; subsequent presses are
     * consumed by main.js itself for sample collection.
     */
    onSpaceDown() {
    ...
    /**
     * Welcome screen: an icon, a short explanation, and a hint that
    ...
    _showIntro() {
        ...
        // Italian user-facing copy left untouched (content, not code).
        box.innerHTML = `
            <div style="font-size: 2.5rem; margin-bottom: 16px;">Calibrazione</div>
    ...
    _showNextPoint() {
        ...
        // animation lives and dies with the overlay (no leftover CSS).
        ...
        // SVG progress ring as a stroked circle. We start fully empty
        // (stroke-dashoffset = perimeter) and decrement towards 0 as
        // samples accumulate, giving the impression of a filling ring.
        // The -90° rotation places the start of the stroke at 12 o'clock.
    ...
    /**
     * Return the current anchor's screen-pixel coordinates. main.js
     * needs them as the (screenX, screenY) target paired with each
     * iris sample fed to the TPS calibrator.
     */
    getNextPointCoords() {
        const p = this.points[this.currentIndex];
    ...
    /**
     * Move on to the next anchor. Called by main.js after the multi-
     * frame sample buffer for the current anchor has been processed
     * and stored in the TPS calibrator.
     */
    advance() {
        this.currentIndex++;
        this._showNextPoint();
